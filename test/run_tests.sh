...
test_usage_mkcert() {
  printLine "Testing mkcert"
  CHECK="$(cat ../Dockerfile | grep -m1 test.verify | sed -e 's/.*test.verify="\(.*\)".*/\1/g')"
  COMMAND="$(cat ../Dockerfile | grep -m1 test.command | sed -e 's/.*test.command="\(.*\)".*/\1/g')"

  printLine "Starting Container"

  OUTPUT=$(docker run --rm ${IMAGE_NAME} "${COMMAND}")
